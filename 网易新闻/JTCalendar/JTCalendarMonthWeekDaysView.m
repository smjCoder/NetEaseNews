...
- (NSArray *)daysOfWeek
{
    ...
    if (self.calendarManager.calendarAppearance.weekDayFormat==1) {
        for (int i=0; i<days.count; i++) {
            if ([days[i] isEqualToString:@"MON"]) {
                [ary addObject:@"一"];
            }
            else if ([days[i] isEqualToString:@"TUE"])
            {
                 [ary addObject:@"二"];
            }
            else if ([days[i] isEqualToString:@"WED"])
            {
                [ary addObject:@"三"];
            }
            else if ([days[i] isEqualToString:@"THU"])
            {
                [ary addObject:@"四"];
            }
            else if ([days[i] isEqualToString:@"FRI"])
            {
                [ary addObject:@"五"];
            }
            else if ([days[i] isEqualToString:@"SAT"])
            {
                [ary addObject:@"六"];
            }
            else if ([days[i] isEqualToString:@"SUN"])
            {
                [ary addObject:@"日"];
            }
        }
